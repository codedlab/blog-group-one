...
const registerUser = async (req, res) =>{
    ...
    const addUser = req.body;
     ...
     if (createUser)
       res.status(201).json({message:"User creat"});
      return;
...
const getUser = async (req, res ) =>  {
    ...
    try {
        ...
        if (getUser)
        res.status(201).json({message:"Successfull", getUser})
        }
    } catch (error) {
        console.log(error)
...
const getallUser = async(req, res) => {
    ...
    try {
        ...
        if (getAllusers){
            res.status(409).json({message:"Successfull", data: getAllusers .length,getAllusers});
            return;
        }
...
// update user;
 ...
 const deleteUser = async (req, res) =>{
    ...
    if (deleteUs){
        ...
        console.log(error)
        return res.status(500).json({message:"unable to connect to the server", error})
